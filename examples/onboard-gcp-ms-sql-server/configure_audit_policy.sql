...
ELSE 
  BEGIN 
    ...
    SET @audit_guid = NEWID()
    SET @sql_command = 'CREATE SERVER AUDIT ' + @server_audit + ' TO FILE (FILEPATH = ''/var/opt/mssql/audit'', MAXSIZE = 2 MB, RESERVE_DISK_SPACE = OFF) WITH (QUEUE_DELAY = 1000, ON_FAILURE = FAIL_OPERATION, AUDIT_GUID = ''' + CAST(@audit_guid AS nvarchar(255)) + ''')';
    EXECUTE(@sql_command);
  END;
