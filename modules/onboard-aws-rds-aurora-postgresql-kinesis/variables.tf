...
variable "aws_aurora_postgresql_cluster_audit_type" {
  ...
  default     = "KINESIS"
  validation {
    condition     = contains(["KINESIS", "KINESIS_AGGREGATED"], var.aws_aurora_postgresql_cluster_audit_type)
    error_message = "Invalida audit_type. Select from \"KINESIS\", \"KINESIS_AGGREGATED\""
  }
}
...
variable "instance_class" {
  ...
  validation {
    ...
    condition = (
      can(regex("^db.r7g.*large$", var.instance_class)) ||
      can(regex("^db.r6g.*large$", var.instance_class)) ||
      can(regex("^db.r6i.*large$", var.instance_class)) ||
      can(regex("^db.r5.*large$", var.instance_class)) ||
      can(regex("^db.x2g.*$", var.instance_class))
    )
...
variable "key_description" {
  ...
  type        = string
  default     = "AWS KMS Key to encrypt Aurora PostgreSQL Database Activity Stream."
}
